...
#This file is part of Syntrax Nova.
...
class NovaWrite:
    def __init__(self, root):
        self.root = root
        self.root.title("Syntrax NovaWrite (Beta 1.01)")
    ...
    def create_toolbar(self):
        ...
        tk.Button(self.toolbar, text="Save", command=self.save_file).pack(side="left", padx=4)

        tk.Label(self.toolbar, text="   ").pack(side="left")

        fonts = list(font.families())
        self.font_box = ttk.Combobox(self.toolbar, values=fonts, width=20)
        self.font_box.set(self.current_font_family)
        self.font_box.pack(side="left")
        tk.Button(self.toolbar, text="Set Font", command=self.set_font_family).pack(side="left")

        self.size_box = ttk.Combobox(self.toolbar, width=5, values=[8,10,12,14,16,18,20,24,28,32])
        self.size_box.set(self.current_font_size)
        self.size_box.pack(side="left")
        tk.Button(self.toolbar, text="Size", command=self.set_font_size).pack(side="left")
        ...
        tk.Button(self.toolbar, text="Color", command=self.choose_color).pack(side="left", padx=6)

        tk.Button(self.toolbar, text="Left", command=lambda: self.align("left")).pack(side="left")
        tk.Button(self.toolbar, text="Center", command=lambda: self.align("center")).pack(side="left")
        tk.Button(self.toolbar, text="Right", command=lambda: self.align("right")).pack(side="left")

        tk.Label(self.toolbar, text="   ").pack(side="left")

        tk.Button(self.toolbar, text="• List", command=self.bullet_list).pack(side="left")
    ...
    def apply_theme(self):
        ...
        self.scroll.configure(bg=scroll_bg, troughcolor=bg, activebackground=btn_bg)
